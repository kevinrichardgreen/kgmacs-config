...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package ibuffer-vc
    :config
    (defun kg/ibuffer-vc ()
      "Ensure buffers get sorted by vc in ibuffer."
      (interactive)
      (ibuffer)
      (ibuffer-vc-set-filter-groups-by-vc-root))
    (global-set-key (kbd "C-x C-b") 'kg/ibuffer-vc))
#+END_SRC
...
Make sure spaceline is set up after the theme you use.
#+BEGIN_SRC emacs-lisp
  (use-package spaceline :after moe-theme
    :ensure t
    :config
...
Set up spaceline with a toggle.
#+BEGIN_SRC emacs-lisp
  (defun kg/toggle-spaceline ()
    "Toggle between no mode-line and spaceline"
    (interactive)
    (if (get 'kg/toggle-spaceline 'state)
        (progn
          (setq mode-line-format nil)
          (put 'kg/toggle-spaceline 'state nil)
          )
      (progn
        (setq mode-line-format '("%e" (:eval (spaceline-ml-main))))
        (put 'kg/toggle-spaceline 'state t))))

  (setq-default mode-line-format nil)
  (global-set-key (kbd "M-s") 'kg/toggle-spaceline)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package avy :after key-chord
    :config
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun kg/open-gnome-terminal ()
    ...
    (if (file-remote-p default-directory)
      ...
      (shell-command sys-terminal-name)))
