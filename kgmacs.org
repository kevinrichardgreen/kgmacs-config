...
#+BEGIN_SRC emacs-lisp
  (use-package key-chord
    :config
    (key-chord-mode +1)
    (defun translate-to-key-chord-lead-key ()
      (interactive)
      (push ?` unread-command-events)
      )
    (global-set-key (kbd "<mouse-3>") 'translate-to-key-chord-lead-key)
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package avy :after key-chord
    ...
    (key-chord-define-global "`f" 'kg/avy-goto-char-ding)
    (key-chord-define-global "`d" 'avy-goto-line))
#+END_SRC
