...
Add an elisp directory to load path for misc packages
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package mixed-pitch
    ...
    ;; useful scaling to get fixed and variable pitch the same size
...
- *Note* do not auto-save history... blows up disk space really quick
- if you do, consider saving it with a directory pattern similar to =~/.emacs_backups/=
#+BEGIN_SRC emacs-lisp
...
- sometimes useful, sometimes not... *there's got to be a better way to set this up for quick toggling*
#+BEGIN_SRC emacs-lisp
...
Custom filter for roam docs:
- This doesn't really work right?
#+begin_src emacs-lisp
(setq ibuffer-saved-filter-groups
        '(("Home"
           ("Roam" (filename . "\\/Documents/roam"))
	   )))
...
Get =TITLE= keyword from an org file.
#+begin_src emacs-lisp
...
Use =TITLE= (if defined) for the name column of org files.
#+begin_src emacs-lisp
  (define-ibuffer-column org-title
    (:name "Name" :inline t)
    (ndk/org-file-get-title filename-and-process))
...
#+BEGIN_SRC emacs-lisp
  (use-package darkroom
    :config
    (global-set-key [f7] 'darkroom-mode)
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'dired
	(setq dired-listing-switches "-lh --group-directories-first") ;; don't list all by default
	;; (setq dired-listing-switches "-lah --group-directories-first") ;; don't list all by default
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package avy
    ...
    (global-set-key (kbd "M-i") 'kg/avy-goto-char-ding)
...
Linum relative mode [[https://github.com/coldnew/linum-relative]].
#+begin_src emacs-lisp
  ;; (use-package linum-relative
  ;;   :config
  ;;   ;; Use the C backend for numbering for performance
...
#+begin_src emacs-lisp
  (use-package telephone-line
    ...
    (setq telephone-line-height 24 ;; give it some space
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;   (minions-mode 1))
#+END_SRC
Note the simplicity of the default modeline with minions. Very info-dense, and still gives quick access to minor modes too.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org-ref
    :config
    (with-eval-after-load 'org
      (setq org-ref-notes-directory "~/pCloudDrive/bison/Documents/notes/reading"
	    org-ref-bibliography-notes "~/pCloudDrive/bison/Documents/bib_notes.org"
	    org-ref-default-bibliography '("~/pCloudDrive/bison/Documents/index.bib")
	    org-ref-pdf-directory '("~/pCloudDrive/bison/Documents/pdf_books/"
				    "~/pCloudDrive/bison/Documents/pdf_papers"))
      (setq bibtex-completion-notes-path "~/pCloudDrive/bison/Documents/bib_notes.org"
	    bibtex-completion-bibliography "~/pCloudDrive/bison/Documents/index.bib"
	    bibtex-completion-library-path '("~/pCloudDrive/bison/Documents/pdf_books/"
				       "~/pCloudDrive/bison/Documents/pdf_papers"))))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    :config
    (setq org-roam-directory (file-truename "~/pCloudDrive/logan/Documents/roam"))
...
*Note:* using an Ubuntu Mate distribution requires in a different
...
#+begin_src emacs-lisp
  (use-package shell-pop
    :config
    (global-set-key (kbd "M-m") 'shell-pop)
  )
#+end_src
...
*** deal.ii highlighting

Major mode for deal.ii parameter file highlighting
#+begin_src emacs-lisp
  (require 'prm-mode)
...
*NOTE*: auctex-latexmk quick fix: https://github.com/tom-tan/auctex-latexmk/pull/40/files
- change to =(require 'tex-buf nil t)= in auctex-latexmk.el
#+begin_src emacs-lisp
...
*** COMMENT TODO Do they work with native-compiled lisp?
#+BEGIN_SRC emacs-lisp
    (use-package elpy
      :config
      (elpy-enable))
