...
DISABLED FOR NOW - trying all navigation with ~avy~.
...
Open files and goto lines like we see from g++ etc. i.e. =filename:line=.
...
#+BEGIN_SRC emacs-lisp
  (use-package avy
    ...
    (defun kg/avy-goto-line-ding ()
     (interactive)
     (ding)
     (call-interactively 'avy-goto-line))
    (key-chord-define-global "jk" 'kg/avy-goto-char-ding)
    (key-chord-define-global "jj" 'kg/avy-goto-char-2-ding)
    (key-chord-define-global "jf" 'kg/avy-goto-line-ding))
#+END_SRC
...
Options for exporting =org-mode= documents.
      ...
      (setq org-latex-default-packages-alist
            (quote
             (("AUTO" "inputenc" t
              ...
              ("" "amssymb" t nil)
              ("" "capt-of" nil nil)
              ("colorlinks,breaklinks" "hyperref" nil nil)))) ; <-- Only change from default
      ;; Additional packages I like
      (setq org-latex-packages-alist
            (quote (("" "etoolbox" nil)                           ; useful for patchcmd
                    ("hyphenbreaks" "breakurl" nil)               ; break long urls
...
capable of everything that =ivy-bibtex= is and then some, but I haven't
taken the time to figure it all out yet. It is very useful for citing
references easily in org documents though. For example, squid axon cite:Hodgkin1952.
...
Rotating pdf pages, courtesy of [[https://emacs.stackexchange.com/a/24766/16286][StackOverflow]].
